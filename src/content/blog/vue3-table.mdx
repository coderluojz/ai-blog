...
description: '手搓一个支持固定头、固定列、列排序、单元格超出隐藏点击展开、上拉加载等功能的 Vue3 移动端表格组件。'
pubDate: 2024-04-22
updatedDate: 2024-04-22
category: 'Vue'
tags: ['Vue', '移动端', '组件开发']
aiSummary: '本文通过具体实战，详细讲解了如何使用 Vue3 构建一个复杂的移动端 Table 组件。涵盖了利用 sticky 实现固定行列、计算偏移量处理多列固定、三态排序状态机设计，以及基于 SelectorQuery 的文本溢出检测机制。'
draft: false
---
...
使用 `fixedColumnsList` 过滤出 `columns` 中 `fixed` 为 `true` 的配置数组，数组中每个对象则为一列，根据过滤出的数组，再进行一个宽度的累加，获取到我们需要粘性定位在表格左侧多少位置比较合适，根据每列对应的`index` and 过滤出来的数据 `fixedColumnsList`计算出来得到 `left` 距离的 `widthCount`，因为我们每一列的 `td` 都是在一个父级里面的，所以需要给每一列的 `td` 设置对应的 `sticky` and `left` 距离。

**增加滚动阴影**

```html
// table-head and table-body 文件中 // 给 table-th and table-td 上增加阴影 // head
...
列排序也是根据 `columns` 中的配置项 `sort` 决定的，下面实现 `handleSort` and `renderSortIconColor` 方法。
