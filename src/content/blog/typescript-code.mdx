...
description: '探讨 TypeScript 类型编程的意义，通过 ParseQueryString 实例展示如何实现更精准的类型提示和检查。'
pubDate: 2022-12-28
updatedDate: 2022-12-28
category: 'TypeScript'
tags: ['TypeScript', '类型编程']
aiSummary: '本文探讨了 TypeScript 类型编程的核心价值。通过实现一个复杂的 ParseQueryString 类型工具，演示了如何利用递归、infer 关键字和映射类型来处理动态字符串，从而在开发中获得极致精准的类型提示。'
draft: false
---
...
类型编程中，我们尽尽可能写出具体的类型可能会更精准的提示出这个变量是什么类型，这样类型提示的精准度高了，在开发中的提交将会更好。
...
这就是需要用到类型编程了，我们可以写一个 `ParseQueryString` 类型：
...
值也需要合并：
如果两个对象类型中都有，那就合并成一个，也就是 `MergeValues<OneParam[Key], OtherParam[Key]>`。
